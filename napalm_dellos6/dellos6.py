...
class DellOS6Driver(NetworkDriver):
    ...
    def get_vlans(self):
        ...
        for vlan_entry in show_vlan:
            ...
            for interface in self._expand_ranges(ports):
                canonical_interfaces.append(canonical_interface_name(
    ...
    def get_mac_address_table(self):
        ...
        get_mac_address_table = textfsm_extractor(self,
                                    "show_mac_address_table",
                                    raw_get_mac_address_table)
        table = []
        for entry in get_mac_address_table:
            table.append({
                "mac"       : mac(entry["mac"]),
                "interface" : canonical_interface_name(entry["port"],
                                    addl_name_map=dellos6_interfaces),
                "vlan"      : mac(entry["vlan"]),
